...
while (answer_term == "no") :
 ...
 if cypage.find("http") >= 0 :
  # query_term = grpage[grpage.rfind('/')+1:]
  query_term = cypage[cypage.rfind('/')+1:]
  print("")
  answer_text = "Έδωσες: " + cypage + ". Είναι σωστό; Πάτα ENTER για ναι. "
...
# Opening files
os.chdir(write_path)
write_file = ("cy-analysi.xls")  # path to xslx write file
alt_write_file = ("cy-analysi_alt.xls")   # alternate name of xls write file
...
if cypage.find("offset") >=0 :
 junk, offs, keeper = cypage.partition('offset=')
 offset = keeper[:keeper.find('&')]
else :
 offset = 0  # starting offset value set to 0 and in each for loop
 ...
 for q in range(0, int(total_next_pages)) :
  ...
  for container in containers :
   ...
   if container.find("font", {"style": "color:#FF0000"}) :
    cy_prod_price = container.find("font", {"style": "color:#FF0000"}).text.strip().replace("\xa0€", "")
   else :
    cy_prod_price = container.find("td", {"class": "web-product-price"}).text.strip().replace("\xa0€", "")
