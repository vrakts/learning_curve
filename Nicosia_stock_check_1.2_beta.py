...
def ti_paizei():
    ...
    # - Ενεργοποίηση του test mode για να μην πειραχτει το αρχικό αρχείο.
    #####################
    # Changelog 1.1
...
try:
    ...
    if s_text == "":
        print_text = "Δεν έχω σπίτι ούτε πατρίδα."
        print("Δεν έχω σπίτι ούτε πατρίδα.")
        win32ui.MessageBox("Δεν έχεις αντιγράψει τον κωδικό.", print_text, win32con.MB_OK)
        sys.exit(0)

    read_path1 = ("K:\\Sales\\Stock")
    read_path2 = ("C:\\Users\\manager\\Desktop")
    read_path3 = ("Z:\\OneDrive\\eShop Stuff\\PRODUCT\\Product")
    retry = 1
    attempt = 0
    path_flag = True
    test_mode = False
    ...
    while path_flag == True:
        try:
            if os.path.exists(read_path1) == True:
                os.chdir(read_path1)
                path_flag = False
            elif os.path.exists(read_path2) == True:
                os.chdir(read_path1)
                path_flag = False
            else:
                os.chdir(read_path3)
            ...
            if attempt == retry:
                win32ui.MessageBox("Exception: " + str(exc),
                                   "Ξεσκιούζ μι...", win32con.MB_OK)
                sys.exit(str(exc))
    ...
    if found == 1:
        print_text = "Υπάρχει " + \
            str(found) + " τεμάχιο " + s_text + " στην περιοχή: " + area + "."
        # print_text = "Υπάρχει " + str(found) + " τεμάχιο " + s_text + " στην περιοχή " + sheet1[i, 10].value + "."
    elif found > 1:
        print_text = "Υπάρχουν " + \
            str(found) + " τεμάχια " + s_text + " στην περιοχή: " + area + "."
    ...
    if found >= 1:
        ...
        win32ui.MessageBox(
            print_text, "Τι να κάνεις; Να μαλώεις;", win32con.MB_OK)
