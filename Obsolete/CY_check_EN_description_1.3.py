...
def set_files() :
 ...
 wb_crazy_write = xlwt.Workbook()  # Create a virtual workbook to keep data in
 ws_crazy_write = wb_crazy_write.add_sheet("en_exist_crazy", cell_overwrite_ok = True)  # add sheet in virtual workbook named after the search string ad run date
...
def write_results(e) :
 if page.find("crazysundays") >= 0 :
  ws_crazy_write(e, 0, cy_code)
 else :
  ws_write.write(e, 0, cy_code)

def write_it_down(crazy_mark) :
 if crazy_mark == True :
  try :
   wb_crazy_write.save(write_file)
   print(write_file + ", το έχω γραμμένο στο " + write_path)
  except :
   wb_crazy_write.save(alt_write_file)
