...
def write_results(e, cy_desc_text) :
 ws_write.write(e, 0, cy_code)
 ws_write.write(e, 1, cy_title)
 if cy_desc_text == "":
  ws_write.write(e, 2, "NO GR")
 else:
  ws_write.write(e, 2, "OK GR")
 if page.find("crazysundays") >= 0 :
  ws_write.write(e, 4, "CRAZY")
 else :
  ws_write.write(e, 4, "-")
...
try :
 ...
 for page in pages_list :
  ...
  for q in range(0, int(total_next_pages)) :
   if crazy_mark == False :
    containers = single_pages(cat_offset_url)
   else :
    containers = single_pages(page)
