...
write_file = (r"K:\SALES\ΧΡΗΣΤΕΣ\PRODUCT 3 - ΓΙΩΡΓΟΣ ΒΡΑΚΑΣ\Script Results\High_Margin_Check_S3.xls")  # path to xslx write file
...
for i in range(1, ac_row):
 if str(sheet[i, 2].value) == "None" :
  ...
  if len(q_stock) != 0 :
   ...
   ws_write.write(i, 0, sheet[i, 2].value)
   ws_write.write(i, 1, "MARGIN")
  else: 
   print("For " + str(sheet[i, 2].value) + " on row " + str(i+1) + " high margin exists")
   ws_write.write(i, 0, sheet[i, 2].value)
   ws_write.write(i, 1, "OK")
