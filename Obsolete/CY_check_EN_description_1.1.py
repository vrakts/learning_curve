...
def get_elapsed_time(e) :
 ...
 print("") 
 if e > 0 :
  input("Found " + str(e) + " non traslated. Press any key when ready to exit.")
 elif e == 0 :
  input("All clear. " + str(e) + " non translated found. Press any key when ready to exit.")
 sys.exit(0)
...
def get_cy_details(container, e) :
 ...
 if cy_d_soup == None or cy_d_soup.text.find('Σύνολο ψήφων') > 0 or cy_product_table_title.text.strip() != "Description" :
  cy_desc_text = ""
  print(cy_code + " description is empty.")
 else :
  ...
  if cy_desc_text.find('Product description is temporary unavailable in English') >= 0 :
   translated = False
   print(cy_code + " not translated.")
   write_results(e)
   e += 1
   # print("e = " + str(e))
  else :
   translated = True
   print(cy_code + " translated.")
 print("")
 return(e)
...
def write_it_down() :
 try :
  ...
  print("Probably the write file is opened. Writing ALT file.")
 ...
 for q in range(0, int(total_next_pages)) :
  ...
  for container in containers :
   ...
   if total_prod - (total_prod - tp) > 0 :
    ...
    print("Processing item: " + str(total_prod - tp) + "/" + str(total_prod) + ".")
   while attempt < 3 :
    try :
