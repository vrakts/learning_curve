...
def get_cy_details(page_soup) :
 global cy_prod_title, cy_price_dif, cy_price_text, cy_avail, cy_availability, cy_cat, cy_subcat, cy_brand, price_dif, pd
 gr_price_dif = '0'
 # pd = 0
...
def choices() :
 global your_choice, query_term, answer_term, upload_file, read_sheet, read_column, gr_url, ws_write, wb_write, write_file
 answer_term = "no"
 ...
 if your_choice == "1" :
  ...
  if query_term.find("://") > 0 :  # if query_term is a full URL then use this as the gr_url.
   ...
   print("Changed filename from " + temp_filename + " to " + filename)
