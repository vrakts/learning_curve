...
# - As a workaround, will try to save the results file on each 10 queries
#	and continue from the there on the next try. To do that it compares
...
for i in range(1, ac_row) :
 ...
 if str(sheet[i, 0].value).strip() == "None" :
  ...
  if len(cy_price) == 0 :  # if the prices table length is zero
   cy_price_text = "NO PRICE"  # then product is out of stock so no price
  else : 
 ...
 if sorry > 0 :
  print("Exceptions caught: " + str(sorry))
 print("")
 # print(str(i))
 # ws_write.write(e,0, str(sheet[i, 0].value.strip()))
 # ws_write.write(e,1, cy_price_text)
