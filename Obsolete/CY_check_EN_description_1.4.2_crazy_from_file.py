...
# - Διαβάζει τα URL από txt file.
# - Αποθήκευση σε κάθε τέλος URL
#####################
# Changelog v1.4
...
def list_pages() :
 global pages_list
 pages_list = []
 if os.path.exists(r"K:\SALES\Stock\Scripts\urlcheck.txt") == True :
...
try :
 ...
 for page in pages_list :
  ...
  for q in range(0, int(total_next_pages)) :
   ...
   for container in containers :
    attempt = 0
    tp = tp - 1
    os.system("title " + "URL: " + str(page_count) + "/" + str(len(pages_list)) + ". " + str(total_prod - tp) + "/" + str(total_prod))
 ...
 write_it_down()
 input("Κάτι δεν πάει καλά. Έσωσα το αρχείο μέχρι εδώ.")
 print(str(exc))
 get_elapsed_time(e)
 sys.exit(0)
