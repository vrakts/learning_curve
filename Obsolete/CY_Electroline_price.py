...
# Electoline website. Will write all values retrieved in an xls file.
#######################
...
write_file = (r"K:\SALES\ΧΡΗΣΤΕΣ\PRODUCT 3 - ΓΙΩΡΓΟΣ ΒΡΑΚΑΣ\Script Results\CY_Electroline_Price_Check.xls")  # path to xslx write file
...
for i in range(1, ac_row):
 if str(sheet[i, 0].value) == "None" :
  ...
  if len(gr_price) == 0 :
   ...
   print("CODE = " + str(sheet[i, 0].value.strip()) + ", εξαντλημένο.")
   ...
   print("CODE = " + str(sheet[i, 0].value.strip()) + ", PRICE = " + gr_price_text)
   ws_write.write(i,0, str(sheet[i, 0].value.strip()))
   ws_write.write(i,1, gr_price_text)
