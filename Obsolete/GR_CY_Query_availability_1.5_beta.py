def ti_paizei() :
 ...
 # και παίρνει την περιγραφή από το GR και διορθώνει όλα τα λάθη.
 ##########################
 # Current Version 1.5 beta
 ##########################
 # Changelog V1.5 beta
 # - Προσθήκη κώδικα μέσα σε defs
 # - Ανανεωμένο σύστημα για τις εξαιρέσεις
 # - Χρήση νέου κώδικα για το άνοιγμα της σελίδας
 # - Χρήση νέου κώδικα για το μέτρημα του χρόνου
...
def choices() :
 global your_choice, answer_term, query_term, upload_file, read_sheet, read_column
 answer_term = "no"
...
def set_files() :
 global write_file, alt_write_file, spreadsheet, ac_row, grpage, wb_write, ws_write, sheets, sheet, write_path
 ...
 if your_choice == "1" :
  ...
  """if os.path.exists(r"K:\SALES\ΑΝΤΑΓΩΝΙΣΜΟΣ\Ανταγωνισμός Λευκωσίας") == True :  # does work folder exist?
   ...
   print("Χρησιμοποιώ το home path 1.")
   print("")
  elif os.path.exists(r"Y:\OneDrive\HTML Parser\PRODUCT 3 - ΓΙΩΡΓΟΣ ΒΡΑΚΑΣ\Script Results") == True :  # does home folder 1 exist?
   write_path = (r"Y:\OneDrive\HTML Parser\PRODUCT 3 - ΓΙΩΡΓΟΣ ΒΡΑΚΑΣ\Script Results")
   print("Χρησιμοποιώ το home path 2.")
   print("")
   ...
   print("Χρησιμοποιώ το home path 3.")
   print("")
  else :
   if os.path.exists(r"C:\TEMPYTH") == True :  # does temp folder exist?
    write_path = (r"C:\TEMPYTH")
    print("Δεν βρέθηκαν προκαθορισμένοι φάκελοι. Χρησιμοποιώ " + write_path)
    print("")
   else :  # if not create it
    os.makedirs(r"C:\TEMPYTH")
    write_path = (r"C:\TEMPYTH")
    print("Δεν βρέθηκαν προκαθορισμένοι φάκελοι. Δημιουργώ και χρησιμοποιώ " + write_path)
    print("")"""
  if query_term.find("://") > 0 :  # if query_term is a full URL then use this as the grpage.
   grpage = query_term  # assign the url entered to the grpage variable
   ...
   grpage = "https://www.e-shop.gr/search?q=" + query_term_2  # this is the base query url for GR
...
def get_totals(page_soup) :
 next_pages_search = page_soup.findAll('td', {'style': 'font-family:tahoma;font-size:14px;padding:0 0 10px 0;'})  # find all next page buttons assuming this is a term based query page
...
def get_offsets(gr_page, page_soup) :
 global page_offset, gr_offset_url, last_offset
 ...
 page_offset = "&t=&c=&offset="  # this holds the offset text of the query page
 gr_offset_url = gr_page + page_offset + str(offset)  # this is the complete query url with offset. eg. https://www.e-shop.gr/search?q=spigen&t=&c=&offset=0
...
try :
 ...
 if your_choice == "1" :  # if this is a predefined file
  ...
  gr_page_soup = load_soup(grpage, wait, retries)
  ...
  # https://www.e-shop.gr/search?q=nilfisk
   total_next_pages = get_totals(gr_page_soup)
   ...
   get_offsets(grpage, gr_page_soup)
   ...
   total_prod = tp = last_offset + len(last_prod_info)
   ...
   if gr_page_soup.findAll('a', {'class': 'mobile_list_navigation_link'}) :  # if the page has next buttons
    ...
    print("Σύνολο σελίδων αναζήτησης: " + str(total_next_pages))
    ...
    gr_last_page_soup = load_soup(gr_last_cat, wait, retries)
    ...
    for q in range(0, int(total_next_pages)) :
     ...
     gr_page_soup = load_soup(gr_cat_offset_url, wait, retries)
