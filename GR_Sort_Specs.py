...
os.chdir(write_path)
write_file = ("GR_Specs_" + filename + ".xls")  # name of xls write file
alt_write_file = ("GR_ALT_Specs_" + filename + ".xls")  # alternate name of xls write file
...
ws_write.write(0, 3, "SORT_SPECS")  # write date on D1 cell
 ...
 if gr_page_soup.findAll('a', {'class': 'mobile_list_navigation_link'}) :  # if the page has next buttons 
  ...
  for q in range(0, int(total_next_pages)) :
   ...
   for container in containers :
    ...
    ##
    gr_prod_sort = container.findAll('td', {'class': 'web-product-info'})[1]
    ...
    if gr_prod_sort.text.find("Αχαρν") > 0:
     ...
     gr_prod_specs = gr_prod_sort.text.replace("Χαρακτηριστικά:", "").replace("\xa0", "").replace(" • ", ",").strip()
     # else :
     # gr_prod_specs = gr_prod_specs[0:gr_prod_specs.find("Αχαρν")+2]
...
print("")
print("File: " + write_path + "\\" + write_file + " saved.")
finished = input("Total products: " + str(e) + ". Ready when you are...")
