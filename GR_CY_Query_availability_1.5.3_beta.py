def ti_paizei() :
 ...
 ############################
 # Changelog V1.5.3 beta
 # - Ψάχνει για την ύπαρξη ενεργειακής ετικέττας στο CY για οθόνες και TV.
 ############################
 # Changelog V1.5.2 beta
 ...
 # Στο παρακάτω link αλλάζει το όνομα του αρχείου σε "1"
 #      https://www.e-shop.cy/search_main?table=ANA&category=%D3%D4%D5%CB%CF&filter-1017=1
 #      https://www.e-shop.cy/search_main?table=TEL&category=%C8%C7%CA%C7&filter-27068=1
 ...
 print("Τρέχουσα έκδοση: 1.5.2 beta.")
...
def get_cy_details(page_soup) :
 global cy_prod_title, cy_price_dif, cy_price_text, cy_avail, cy_availability, cy_cat, cy_subcat, cy_brand, price_dif, cy_energy, pd
 gr_price_dif = '0'
 # pd = 0
 ...
 if len(cy_prod_title) == 0 :
  ...
  if page_soup.find("td", {"style" : "text-align:left;padding:5px 0 2px 5px;color:#4f4f4f;font-family:Tahoma;font-size:14px;font-weight:bold;"}):
   avail = page_soup.find("td", {"style" : "text-align:left;padding:5px 0 2px 5px;color:#4f4f4f;font-family:Tahoma;font-size:14px;font-weight:bold;"})
   avail_text = avail.text
   if avail_text.find('ËÅÌ:') >= 0 :
    avail_text_lim = avail_text[avail_text.find('ËÅÌ: ')+5:avail_text.find('ËÅÕ: ')-1].strip()
    avail_text_nic = avail_text[avail_text.find('ËÅÕ: ')+5:avail_text.find('ËÁÑ: ')-1].strip()
    avail_text_lar = avail_text[avail_text.rfind(': ')+1:].strip()
   else :
    avail_text_lim = avail_text[avail_text.find('ΛΕΜ: ')+5:avail_text.find('ΛΕΥ: ')-1].strip()
    avail_text_nic = avail_text[avail_text.find('ΛΕΥ: ')+5:avail_text.find('ΛΑΡ: ')-1].strip()
    avail_text_lar = avail_text[avail_text.rfind(': ')+1:].strip()
   cy_availability = int(avail_text_lim) + int(avail_text_nic) + int(avail_text_lar)
  ...
  if len(cy_categories) == 0 :
   ...
   cy_brand = "-"
  try:
   if cy_cat == "ΟΘΟΝΗ" or cy_cat == "ΤΗΛΕΟΡΑΣΗ":
    energy_link = page_soup.find("td", {"class":"product_table_body"}).a['href']
    if energy_link.find("energy_saving") >= 0:
     energy_class = page_soup.find("td", {"class":"product_table_body"}).img["src"]
     cy_energy_temp = energy_class[energy_class.rfind(".") - 1]
     if cy_energy_temp != "A" and cy_energy_temp != "B" and cy_energy_temp != "C" and cy_energy_temp != "D" and cy_energy_temp != "E" and cy_energy_temp != "F" and cy_energy_temp != "G":
      cy_energy = "YES"
     else:
      cy_energy = cy_energy_temp
    else:
     cy_energy = "NO"
   else:
    cy_energy = "-"
...
def write_results(e) :
 ...
 try :
  ...
  ws_write.write(e, 16, "")
 ws_write.write(e, 17, cy_availability)	# OK
 ws_write.write(e, 18, cy_energy)   	# OK
...
def choices() :
 global your_choice, query_term, answer_term, upload_file, read_sheet, read_column, gr_url, ws_write, wb_write, write_file, alt_write_file, sheet, ac_row
 answer_term = "no"
 ...
 if your_choice == "1" :
  ...
  if query_term.find("://") > 0 :  # if query_term is a full URL then use this as the gr_url.
   ...
   print("Άλλαξα το όνομα του αρχείου από " + temp_filename + " σε " + filename)
  ...
  filename = filename.replace("+", " ")
  print("Τελικό όνομα αρχείου: " + filename)
 ...
 ws_write.write(0, 17, "STOCK")		# write STOCK on R1 cell
 ws_write.write(0, 18, "ENERGY")	# write ENERGY on S1 cell
...
def help() :
 print("")
 os.system('cls')
 print(" Καλώς τον. Χρησιμοποίησε αυτό το πρόγραμμα για να κάνεις σύγκριση κατηγοριών, όρων αναζήτησης ή προκαθορισμένων αρχείνω με λίστες προϊόντων ανάμεσα στο CY και στο GR site.")
 print(" Φυσικά δουλεύει καλύτερα μέσα στο δίκτυο της εταιρείας.")
 print(" Μπορείς να χρησιμοποιήσεις τις παρακάτω επιλογές στην γραμμή εντολής για να αλλάξεις την ροή εκτέλεσης του προγράμματος.")
 print("")
 print("")
 print(" Λίστα επιλογών:")
 print("")
 print("")
 print("*** -help ή -h -?: Εμφανίζει αυτή την οθόνη με πληροφορίες για τις επιλογές.")
 print("")
 print("*** -c:[x] ή -C:[x] - Προκαθορίζει τις επιλογές: 1 = Λίστα από αρχείο excel, 2. URL ή ορος αναζήτησης.")
 print("    πχ. GR_CY_Query_availability -c:1")
 print("")
 print("*** -u:[URL] ή -U:[URL] - Εφόσον έχει οριστεί σωστά η επιλογή '-c:2' προσθέτει τον σύνδεσμο αναζήτησης αλλιώς θα αγνοηθεί τελείως η επιλογή.")
 print("    πχ. GR_CY_Query_availability -c:1 -u:https://www.e-shop.gr/ypologistes-kartes-grafikon-gpu-list?table=PER&category=%CA%C1%D1%D4%C1+%C3%D1%C1%D6%C9%CA%D9%CD")
 ...
 if len(sys.argv) > 1 :
  ...
  for arg in sys.argv :
   ...
   if arg.find("-c:") == 0 or arg.find("-C:") == 0 :
    print("Found -c")
    selection = arg[arg.find(":"):].replace(":", "")
    c_sel = selection
   if arg.find("-u:") == 0 or arg.find("-U:") == 0 :
    print("Found -u")
    selection = arg[3:].replace(":", "")
    u_sel = selection
...
try :
 ...
 if your_choice == "1" :  # if this is a predefined file
  ...
  if gr_page_soup.find('h1', {'style': 'display:inline;font-size:16px;font-family:tahoma;font-color:inherit;'}) or gr_page_soup.find('h2', {'style': 'display:inline;font-size:16px;font-family:tahoma;font-color:inherit;'}) :  # σάμπως η σελίδα που δώσαμε είναι αρχική της κατηγορίας και όχι όλα τα προϊόντα;
