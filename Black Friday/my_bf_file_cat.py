...
write_file = "My_file_cat.xls"  # path to xslx write file
alt_write_file = "My_file_cat_ALT.xls"  # path to xslx write file
...
ws_write.write(0, 4, 'CATEGORY')  # write date on D1 cell
ws_write.write(0, 5, 'BRAND')  # write date on D1 cell
ws_write.write(0, 6, 'AVAIL')  # write date on E1 cell
...
for i in range(1, ac_row):
 ...
 if sheet[i, 0].value != sheet[i-1, 0].value :
  ...
  cy_cat = cy_categories[1].text[:cy_categories[1].text.find(' •')].strip()
  cy_brand = cy_categories[1].text[cy_categories[1].text.find(' •')+2:cy_categories[1].text.find('στην')].strip()
  cy_code = sheet[i, 0].value.strip()
  cy_title = sheet[i, 2].value
  if sheet[i, 4].value == None :
   cy_b_price = 'No Price'
  else :
   cy_b_price = str(sheet[i, 4].value)
  cy_avail = str(int(sheet[i, 5].value))
  ws_write.write(e, 0, cy_code)  # code
  ws_write.write(e, 1, cy_title)  # title
  ws_write.write(e, 2, cy_price_text)  # current price
  ws_write.write(e, 3, cy_b_price)  # black friday price
  ws_write.write(e, 4, cy_cat)  # product category
  ws_write.write(e, 5, cy_brand)  # product brand
  ws_write.write(e, 6, cy_avail)  # availability
  print("Product: " + cy_code + " - " + cy_title)
  print("Current Price: " + cy_price_text + ", Black Price: " + cy_b_price)
  print("Category: " + cy_cat + ", Brand: " + cy_brand + ", Stock: " + cy_avail)
  print("")
 else :
