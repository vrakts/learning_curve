...
cypage = 'https://www.eshopcy.com.cy/search_main.phtml?table=PER'
...
if cy_page_soup.findAll('table', {'class': 'web-product-container'}) :  # search result page with categories
# https://www.e-shop.gr/ilektrikes-syskeues-ilektrikes-skoupes-1001w-eos-1200w-list?table=HAP&category=%C7%CB%C5%CA%D4%D1%C9%CA%C5%D3+%D3%CA%CF%D5%D0%C5%D3&filter-12563=1
 ...
 if cy_page_soup.findAll('a', {'class': 'mobile_list_navigation_link'}) :  # if the page has next buttons 
  ...
  cy_cat_page, query_mark, categories = str(grpage).partition("?")
  ...
  while attempt < 3 :
   try :
    ...
    cy_last_page_soup = soup(gr_last_uClient.read(), "html5lib")
  ...
  for q in range(0, int(total_next_pages)) :
   ...
   for container in containers :
    ...
    print("Code: " + cy_prod_per + ", Title: " + cy_prod_title + ", Price: " + cy_prod_price)
