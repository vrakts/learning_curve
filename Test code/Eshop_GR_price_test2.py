...
from bs4 import BeautifulSoup as soup  # HTML data structure reader
from urllib.request import urlopen as uReq  # Web page reader

# Set page, read it, soup it and close connection.
...
# attempt to grab all products info on <td valign="middle">
prod_info =  page_soup.findAll("td", {"valign": "middle"})
prod_info[1].a['href'] # product link
prod_info[1].a.text # product title
prod_info[1].span.text # product PER code
prod_info[1].find("style", "font-size:14px;font-family:tahoma;color:#900100;width:120px;border-bottom:#909090 1px solid;")
...
for i in prod_info:
 prod_link = i.a['href']
 prod_title = i.a.text
 prod_per = i.span.text
 print(prod_title + " - " + prod_per )
 price_text = prod_price[1].text # save text of the price result in price_text
 if price_text.count(' ') > 1:	# if price " " is more than 1 then it has a discount ... 
 price_text[price_text.find(' ')+1:].replace(" €","")	# ... so print the second price without the euro sign
else:
