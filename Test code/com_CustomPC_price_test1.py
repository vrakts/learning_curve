...
read_file = (r'Z:\Users\Vrakts\Desktop\Ανταγωνισμός Λευκωσίας\CUSTOMPC - ΒΛΑΔΙΜΗΡΟΣ.ods')  # path to ods read file
...
write_file = (r"Z:\Users\Vrakts\Desktop\Html Parser - Python\CustomPC_Price_Check.xls")  # path to xslx write file
...
for i in range(1, ac_row):
 if str(sheet[i, 2].value) == "None" :
  ...
  page_url = "https://www.custompc.com.cy/search/?subcats=Y&pcode_from_q=Y&pshort=Y&pfull=Y&pname=Y&pkeywords=Y&search_performed=Y&scode_from_q=Y&cid=0&q=" + pure_code
  uClient = uReq(page_url)
  page_soup = soup(uClient.read(), "html.parser")
  uClient.close()
  cp_price = page_soup.findAll("span", {"id" : re.compile('sec_discounted_price*')})
  if len(cp_price) == 0 :
   cp_price = "Εξαντλημένο"
   print("CODE = " + pure_code + ", εξαντλημένο.")
   ws_write.write(i,0, pure_code)
   ws_write.write(i,1, cp_price_text)
  else : 
   cp_price_text = cp_price[0].text.replace("\xa0€","").replace(".", ",")
   print("CODE = " + pure_code + ", PRICE = " + cp_price_text)
   ws_write.write(i,0, pure_code)
   ws_write.write(i,1, cp_price_text)
